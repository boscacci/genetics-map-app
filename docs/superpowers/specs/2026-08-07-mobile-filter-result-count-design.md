...
The count uses the same filtering function and filtered list sent through `onFilterChange`, including the committed name query. Typing a name without pressing Go/Enter does not claim that the map has already filtered; pressing Go/Enter updates both the map and the status together. The existing zero-match global-map fallback remains unchanged; the status explicitly reports `No results`.
...
- Extract or reuse one filtering path so the displayed count cannot diverge from the filtered list sent through `onFilterChange`.
